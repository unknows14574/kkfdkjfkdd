...
function Core.SavePlayer(xPlayer, cb)
	MySQL.prepare('UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `job_service` = ?, `job2` = ?, `job2_grade` = ?, `job2_service` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?', {
...
function Core.SavePlayers(cb)
	...
	if count > 0 then
		...
		MySQL.prepare("UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `job_service` = ?, `job2` = ?, `job2_grade` = ?, `job2_service` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ? WHERE `identifier` = ?", parameters,
		function(results)
			if results then
...
function ESX.GetIdentifier(playerId, choice)
	local license = nil
	...
	if not choice then
		...
		if choice == 'license' then
			return license
		elseif choice == 'steam' then
			return identifier
		elseif choice == 'discord' then
			return discord
		end
	end
end
...
function ESX.RegisterUsageUsableItem(usage_name, cb)
	ESX.UsageUsableItemsCallbacks[usage_name] = cb
end

function ESX.UseUsageItem(xPlayer, usage)
	ESX.UsageUsableItemsCallbacks[usage.type](xPlayer, usage)
end
...
function ESX.GetUsableItem(item)
	local usableCallback = Core.UsableItemsCallbacks[item]
	return usableCallback ~= nil and usableCallback ~= {}
end
