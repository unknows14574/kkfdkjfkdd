...
-- 	local menu = RageUI.CreateMenu(ESX.PlayerData.firstname, "" .. tonumber(InvHeight) .. "KG/50.0KG", nil, nil, nil, nil, nil, 128, 0, 128, 155)
-- 	menu:SetStyleSize(300)
-- 	local submenu = RageUI.CreateSubMenu(menu, "Sous Menu", "Choisir", nil, nil, nil, nil, nil, 128, 0, 128, 155)
-- 	local submenu2 = RageUI.CreateSubMenu(menu, "Sous Menu", "Choisir", nil, nil, nil, nil, nil, 128, 0, 128, 155)
...
-- 	local options = {
-- 		{name = "Utiliser", subname = "Utiliser", submenu = menu},
-- 		{name = "Donner", subname = "Donner", submenu = submenu2},
-- 		{name = "Jeter", subname = "Jeter", submenu = menu},
-- 	}
...
-- 				local formattedMoney = "$" .. ESX.Math.GroupDigits(ESX.PlayerData.money)
		
-- 				table.insert(inventory, {
-- 					label     = ('%s: <span style="color:white;">%s</span>'):format(_U('cash'), formattedMoney),
-- 					name      = formattedMoney,
...
-- 					table.insert(inventory, {
-- 						label     = ('%s: <span style="color:white;">%s</span>'):format(ESX.PlayerData.accounts[i].label, formattedMoney),
-- 						name      = ESX.Math.GroupDigits(ESX.PlayerData.accounts[i].money),
...
-- 			for i=1, #ESX.PlayerData.usages, 1 do
-- 				table.insert(inventory, {
-- 					label     = ESX.PlayerData.usages[i].name .. " " .. ESX.PlayerData.usages[i].label,
-- 					name      = ESX.PlayerData.usages[i].label,
-- 					count     = 1,
-- 					type      = 'item_usable',
-- 					value     = ESX.PlayerData.usages[i],
-- 					usable    = true,
-- 					rare      = false,
-- 					canRemove = false
-- 				})
...
-- 						name 	  = ESX.PlayerData.inventory[i].label,
...
-- 				end
-- 			end
-- 			for line, action in pairs(inventory) do
-- 				RageUI.Button(action.label, action.label, {RightLabel = "→→→"}, true, {
-- 					onSelected = function()
-- 						Infos = {
-- 							label = action.label,
-- 							name = action.name,
...
-- 			local player, distance = ESX.Game.GetClosestPlayer()
-- 			for line, action in pairs(options) do
-- 				if Infos.canRemove then
-- 					if action.name == "Donner" then
-- 						if player ~= -1 and distance <= 3.0 then
-- 							RageUI.Button(action.name, action.name, {RightLabel = "→→→"}, true, {
-- 								onSelected = function() submenu2:SetTitle("Donner") end
-- 							}, action.submenu)
-- 						end
-- 					elseif action.name == "Jeter" then
-- 						RageUI.Button(action.name, action.name, {RightLabel = "→→→"}, true, {
-- 							onSelected = function()
...
-- 								if Infos.type == 'item_weapon' then
-- 									TriggerServerEvent('esx:removeInventoryItem', Infos.type, Infos.value)
-- 								else
...
-- 											TriggerClientEvent('Core:ShowNotification', "Quantité invalide")
-- 										else
-- 											TriggerServerEvent('esx:removeInventoryItem', Infos.type, Infos.value, quantity)
-- 											menu2.close()
-- 										end
...
-- 					end
-- 				end
-- 				if Infos.usable then
-- 					if action.name == "Utiliser" then
-- 						if Infos.type == "item_usable" then
-- 							RageUI.Button(action.name, action.name, {RightLabel = "→→→"}, true, {
-- 								onSelected = function() 
-- 									TriggerServerEvent('esx:UsageUseItem', Infos.value)
-- 								end
-- 							}, action.submenu)
-- 						else
-- 							RageUI.Button(action.name, action.name, {RightLabel = "→→→"}, true, {
-- 								onSelected = function() 
-- 									TriggerServerEvent('esx:useItem', Infos.value)
-- 								end
-- 							}, action.submenu)
-- 						end
-- 					end
...
-- 				TriggerClientEvent('Core:ShowNotification', "Aucun joueur a proximité.")
-- 				return
-- 			else
-- 				TriggerEvent('esx_showid', elements2)
-- 			end
-- 			foundPlayers = false


-- 			for line, action in pairs(elements2) do
-- 				RageUI.Button(action.label, action.label, {RightLabel = "→→"}, true, {
-- 					onSelected = function()
...
-- 								if players[i] == action.player then
-- 									foundPlayers = true
...
-- 								TriggerClientEvent('Core:ShowNotification', "Vous ne pouvez pas donner quelque chose à quelqu'un dans un véhicule.")
-- 								return
-- 							end

-- 							TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), Infos.type, Infos.value, sourceAmmo, (ESX.GetWeaponLabel(Infos.value) or Infos.value))
-- 							TriggerEvent('esx_showid', false)
-- 						elseif Infos.type == 'item_usable' then
-- 							local closestPed = GetPlayerPed(nearbyPlayer)
...
-- 							TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), Infos.type, Infos.value, 1, Infos.value)
-- 							TriggerEvent('esx_showid', false)
-- 						else
-- 							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'inventory_item_count_give', {
-- 								title = _U('amount')
...
-- 								if quantity ~= nil then
-- 									TriggerServerEvent('esx:giveInventoryItem', GetPlayerServerId(nearbyPlayer), Infos.type, Infos.value, quantity, (ESX.GetWeaponLabel(Infos.value) or Infos.value))
-- 									TriggerEvent('esx_showid', false)
-- 									menu3.close()
-- 								else
-- 									TriggerClientEvent('Core:ShowNotification', "Quantité invalide")
